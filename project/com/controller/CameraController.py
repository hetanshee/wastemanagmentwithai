    ...
    try:
        if adminLoginSession() == "admin":
            ...
            return render_template(
                "admin/addCamera.html", zoneVOList=zoneVOList, areaVOList=areaVOList
            )
        else:
...
def adminInsertCamera():
    try:
        if adminLoginSession() == "admin":
            ...
            out = cv2.VideoWriter(
                output_video, fourcc, 5.0, (frame_width, frame_height)
            )
            print("4")
            while cap.isOpened():
                ret, image = cap.read()
                ...
                output = imutils.resize(
                    orig, width=frame_width
                )  # , height=frame_height)
            ...
            cameraVO.cameraOutputFilePath = cameraOutputFilePath.replace(
                "project", ".."
            )
...
def adminDeleteCamera():
    try:
        if adminLoginSession() == "admin":
            ...
            cameraInputFilePath = cameraList.cameraInputFilePath.replace(
                "..", "project"
            )
            print("3333333333333")

            cameraOutputFileName = cameraList.cameraOutputFileName
            cameraOutputFilePath = cameraList.cameraOutputFilePath.replace(
                "..", "project"
            )
            print("111111111111")
            print(cameraOutputFilePath)
...
def adminEditCamera():
    try:
        if adminLoginSession() == "admin":
            ...
            return render_template(
                "admin/editCamera.html",
                zoneVOList=zoneVOList,
...
def adminViewNotification():
    try:
        if adminLoginSession() == "admin":
            ...
            return render_template(
                "admin/viewNotification.html", imageVOList=imageVOList
            )
        else:
            adminLogoutSession()
